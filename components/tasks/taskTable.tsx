                        ...
                        {body.map((row, rowIndex) => (
                            <tr key={rowIndex} className={rowIndex % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                                {headers.map((header) => {
                                    ...
                                    const cellContent = typeof cellData === 'string' || typeof cellData === 'number' ? cellData : String(cellData);

                                    if (header.key === 'status') {
                                        return (
                                            <td
                                                key={header.key}
                                                className={`px-4 py-3 text-sm font-medium ${getStatusColor(cellContent as string)}`}
                                            >
                                                {cellContent}
                                            </td>
                                        );
